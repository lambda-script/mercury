...
- **Detector interface** (`src/detector/index.ts`): Abstracts language detection. Only implementation is franc-based (`franc.ts`). For short text (< `minDetectLength`), uses Unicode script-based detection (Hiragana/Katakana → Japanese, Hangul → Korean, CJK → Chinese, etc.) before falling back to "undetermined". Longer text uses franc's trigram analysis, with a kana-presence override that flips `cmn` → `jpn` because franc frequently misidentifies kanji-heavy Japanese as Chinese.
- **Translator interface** (`src/translator/index.ts`): Abstracts translation backends. Two implementations: `google-free.ts` (no API key, default) and `haiku.ts` (uses Claude Haiku via Anthropic SDK). The haiku client pins `baseURL` to `https://api.anthropic.com` to prevent recursion if the user has configured an upstream Claude-API proxy that routes through Mercury.
- **google-free resilience** (`src/translator/google-free.ts`): Text is chunked at paragraph/sentence boundaries (max 4500 chars) to stay under Google Translate's ~5000 char limit; chunks are paired with their original separators so whitespace is preserved on reassembly. Each chunk retries up to 3 times with TLD rotation (`com`, `co.jp`, `co.uk`) and exponential backoff, with a 15s per-attempt timeout so a hung HTTPS connection cannot freeze the serial translation queue. On total failure, returns original text (graceful degradation).
